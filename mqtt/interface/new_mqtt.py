...
class MqttClient:
    def __init__(self, host=None, port=None, topic=None, user=None, password=None, qos=0, enable_random_client_id=False):
        self.host = host
        self.port = port
        self.topic = topic
        self.user = user
    ...
    def exec_write(self, payload: str):
        try:
            while not self.client.is_connected():
                time.sleep(1)
                print('mqtt is disconnected. wait reconnect.')

            result = self.client.publish(self.topic, payload, self.qos)
